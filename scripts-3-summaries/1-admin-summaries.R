...
source('scripts-3-summaries/# table-functions.R')
...
soils <- terra::aggregate(terra::rast(paste0(
  input_path, 'soilgrids_properties_cropland.tif'
)), 10, mean, na.rm = T)
l_ph_h_hp <- terra::ifel(soils$ph < 5.5 &
                           soils$hp_sat >= 10, 1, NA)
names(l_ph_h_hp) <- 'l_ph_h_hp'
l_ph_l_hp <- terra::ifel(soils$ph < 5.5 &
                           soils$hp_sat < 10, 1, NA)
names(l_ph_l_hp) <- 'l_ph_l_hp'
h_ph_h_hp <- terra::ifel(soils$ph >= 5.5 &
                           soils$hp_sat >= 10, 1, NA)
names(h_ph_h_hp) <- 'h_ph_h_hp'
h_ph_l_hp <- terra::ifel(soils$ph >= 5.5 &
                           soils$hp_sat < 10, 1, NA)
names(h_ph_l_hp) <- 'h_ph_l_hp'
...
lime_data <- lapply(crop_names, function(crop) {
  ...
  lime_c <- area_c * lime_c
  names(lime_c) <- paste0(crop, '_lr_tha')
  lime_c
})
for (sa in select_admin) {
  for (country_iso3 in countries_iso) {
...
for (sa in select_admin) {
  for (country_iso3 in countries_iso) {
  extract_profit(country_iso3, sa, save_map = T)
  }
}
