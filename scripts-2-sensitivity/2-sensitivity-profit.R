...
# run profitability
profit_calc <- function(profit_type = 'year1',
                        cprice_factor = 1,
                        yield_factor = 1,
                        lime_price = 100,
                        discount_rate = 0.1) {
  ...
  for (r_f in profit_type) {
    ...
    for (crop in unique(crops_df$spam)) {
      ...
      crop_yield_q <- terra::rast(Sys.glob(
        paste0(
          input_path,
          "yield_quantiles/",
          crop,
          "_spam_yield_quantiles.tif"
        )
      ))
