...
yield_quantiles <- function(crop, sd_f, quantiles, n_samples) {
  ...
  crop_yield$sd <- terra::ifel(crop_yield$spam > 0,
                               sd_f * terra::global(crop_yield, fun = 'sd', na.rm = T)$sd,
                               NA)
  ...
  gauss_dist$fitted <- lapply(seq_len(nrow(gauss_dist)), function(i) {
    truncnorm::rtruncnorm(
      n = n_samples,
      a = 0,
      b = max_crop_yield,
      mean = gauss_dist$spam[i],
      sd = gauss_dist$sd[i]
    )
  })
  gauss_dist$q <- lapply(gauss_dist$fitted, function(x)
  ...
  crop_yield_q <- terra::rast(
    gauss_dist,
    extent = terra::ext(crop_yield),
    crs = terra::crs(crop_yield)
  )
...
# run function
yield_quant <- lapply(names(spam_yield), function(crop) {
  print(crop)
  yield_quantiles(
    crop = crop,
    sd_f = sd_f,
...
names(yield_quant) <- names(spam_yield)
