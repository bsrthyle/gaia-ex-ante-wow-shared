...
# loss
resp_hp <- terra::rast(Sys.glob(
  paste0(input_path, 'ecocrop_yieldloss/hp_crop_suitability_*_0.tif')
))
names(resp_hp) <- gsub("_hp", "", names(resp_hp))
resp_hp <- terra::aggregate(resp_hp, 10, fun = 'mean', na.rm = T)
resp_ph <- terra::rast(Sys.glob(
  paste0(input_path, 'ecocrop_yieldloss/ph_crop_suitability_*_0.tif')
))
...
fao_price <- subset(fao_price, Item %in% crops_df$Item &
                      Area %in% country$Area)
...
crop_price <- aggregate(
  fao_price$Value,
  by = list('crop' = fao_price$spam),
  FUN = median,
  na.rm = T
) # median price

# lime per tas
merlos <- terra::aggregate(terra::rast(paste0(input_path, 'caco3_merlos.tif')), 10, mean, na.rm =
                             T)
merlos_m <- terra::aggregate(terra::rast(paste0(
  input_path, 'caco3_merlos_maintenance.tif'
)), 10, mean, na.rm = T)

# lime per crop
soil <- terra::aggregate(terra::rast(paste0(
  input_path, 'soilgrids_properties_all.tif'
)), 10, 'mean', na.rm = T)
...
lr_crops <- terra::rast(lapply(crops_df$spam, function(crp) {
  lr(crop = crp, lime_method = merlos)
}))
lr_m_crops <- terra::rast(lapply(crops_df$spam, function(crp) {
  lr(crop = crp, lime_method = merlos_m)
}))
...
returns <- function(crop, yield_resp, crop_price, yield_f) {
  ...
  soil_hp <- terra::ifel(soil$hp_sat > c_subset$ac_sat, 1, NA)
  yield_resp_tha <- terra::subst(yield_resp_tha, 0, NA)*soil_hp
  names(yield_resp_tha) <- paste0(crop, '_yresp_tha')
  return_usha <- yield_resp_tha * crop_price* soil_hp
...
nyears_f <- function(crop, lime_year1, lime_maint) {
  ...
  nyears <- round(lime_1 / lime_m, 0)
  names(nyears) <- 'nyears'
  return(nyears)
}

# profitability
profit <- function(crop,
                   yield_resp,
                   yf,
                   crop_price,
                   returns_f = c('year1', 'npv', 'equilibrium'),
                   discount_rate,
                   lime_method,
                   lime_m_method,
                   lime_price,
  ...
  if (returns_f == 'year1') {
    ...
    return <- limer::NPV_lime(return_1[[paste0(crop, "_return_usha")]], nyears =
                                nyrs, discount_rate = discount_rate)
  ...
  gm <- return[[paste0(crop, "_return_usha")]] - cost[[paste0(crop, "_cost_usha")]]
  names(gm) <- paste0(crop, '_gm_usha')
  roi <- return[[paste0(crop, "_return_usha")]] / cost[[paste0(crop, "_cost_usha")]]
  ...
  for (crop in unique(crops_df$spam)) {
    ...
    if (r_f == 'npv') {
      ...
      crop1 <- profit(
        ...
        discount_rate = 0.10,
        lime_method = lr_crops,
        lime_m_method = lr_m_crops,
        lime_price = 100
      )
    }
    crop2 <- c(area_ha, crop1)
    terra::writeRaster(crop2,
                       paste0(input_path, 'profit_baseline/', crop, '_', r_f, '.tif'),
                       overwrite = T)
  }
}
